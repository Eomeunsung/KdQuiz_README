...
키득퀴즈(KdQuiz)는 졸업작품으로 진행된 퀴즈 기반 학습 플랫폼입니다. 처음으로 웹 서버를 개발하며 RESTful API와 데이터 베이스 연동 등 기본적인 서버 구조를 익히고, 퀴즈 생성·수정·삭제 및 참여자 관리 기능을 구현하였습니다. 원래 계획했던 WebSocket 기반 실시간 플레이 기능은 시간과 경험 부족으로 구현하지 못했지만, 대신 각 플레이어가 퀴즈를 풀고 점수를 DB에 저장하도록 시스템을 수정하여 퀴즈 종료 시 점수와 랭킹을 확인 할 수 있도록 기능을 완성하였습니다. 이를 통해 웹 서버 개발 경험과 데이터 흐름 능력을 쌓을 수 있었습니다.
